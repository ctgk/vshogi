...
{
    auto node_l = Node(true, Move(), zero, inf);
...
{
    auto node_l = Node(true, Move(), inf, zero);
...
{
    auto node_l = Node(false, Move(), inf, zero);
    CHECK_TRUE(node_l.proved_mate());
    auto g = Game();
    auto n = Node(false, Move());
    CHECK_FALSE(n.simulate(g, nullptr, nullptr, &node_l));
...
{
    auto node_l = Node(false, Move(), zero, inf);
    CHECK_TRUE(node_l.proved_no_mate());
    auto g = Game();
    auto n = Node(false, Move());
    CHECK_TRUE(n.simulate(g, nullptr, nullptr, &node_l));
...
{
    auto twin = Node(true, Move(), zero, inf);
...
{
    auto twin = Node(true, Move(), inf, zero);
...
{
    auto twin = Node(false, Move(), inf, zero);
    CHECK_TRUE(twin.proved_mate());
    auto g = Game();
    auto n = Node(false, Move());
    CHECK_TRUE(n.simulate(g, nullptr, &twin));
...
{
    auto twin = Node(false, Move(), zero, inf);
    CHECK_TRUE(twin.proved_no_mate());
    auto g = Game();
    auto n = Node(false, Move());
    CHECK_FALSE(n.simulate(g, nullptr, &twin));
...
TEST(dfpn3_node_expand, offence_no_twins)
{
    ...
    n.expand(Game("2B1k/5/3P1/3GK/4R b P"));
...
{
    auto twin_g = Node();
    twin_g.expand(Game("2B1k/5/4p/5/5 b RP"));
    auto n = Node();
    CHECK_FALSE(n.fully_expanded());
    n.expand(Game("2B1k/5/4p/5/5 b P"), &twin_g);
    CHECK_TRUE(n.fully_expanded());
...
{
    auto twin_l = Node();
    twin_l.expand(Game("2B1k/5/4p/5/5 b P"));
    auto n = Node();
    CHECK_FALSE(n.fully_expanded());
    n.expand(Game("2B1k/5/4p/5/4R b SP"), nullptr, &twin_l);
    CHECK_TRUE(n.fully_expanded());
...
TEST(dfpn3_node_expand, defence_no_twins)
{
    auto n = Node(false, Move());
    CHECK_FALSE(n.fully_expanded());
    n.expand(Game("3gk/5/4R/5/4K w p"));
    CHECK_TRUE(n.fully_expanded());
...
{
    auto twin_l = Node(false, Move());
    twin_l.expand(Game("3gk/5/4R/5/4K w p"));
    auto n = Node(false, Move());
    CHECK_FALSE(n.fully_expanded());
    n.expand(Game("3gk/5/4R/5/4K w ps"), nullptr, &twin_l);
    CHECK_TRUE(n.fully_expanded());
    std::set<uint16_t> moves;
    for (auto ch = n.get_child(); ch; ch = ch->get_sibling()) {
        CHECK_TRUE(ch->offence());
...
{
    auto twin_g = Node(false, Move());
    twin_g.expand(Game("3gk/5/4R/5/4K w ps"));
    auto n = Node(false, Move());
    CHECK_FALSE(n.fully_expanded());
    n.expand(Game("3gk/5/4R/5/4K w s"), &twin_g);
    CHECK_TRUE(n.fully_expanded());
    std::set<uint16_t> moves;
    for (auto ch = n.get_child(); ch; ch = ch->get_sibling()) {
        CHECK_TRUE(ch->offence());
...
TEST(dfpn3_node_backprop, defence_preference)
{
    ...
    auto n = Node(false, Move(SQ_5B, HI));
    n.expand(g);
...
TEST(dfpn3_node_select, offence_with_disproved_child)
{
    ...
    auto c = n.select(inf, inf, th_p_ch, th_d_ch);
    g.apply_dfpn(c->get_action());
    auto twin_l = Node(false, Move(), zero, inf);
