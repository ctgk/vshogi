...
constexpr u32 MAX_FEATURES1_NUM
    = PIECETYPE_NUM /*駒の配置*/ + PIECETYPE_NUM /*駒の利き*/
      + MAX_ATTACK_NUM /*利き数*/;
...
constexpr u32 MAX_PIECES_IN_HAND_SUM
    = MAX_HPAWN_NUM + MAX_HLANCE_NUM + MAX_HKNIGHT_NUM + MAX_HSILVER_NUM
      + MAX_HGOLD_NUM + MAX_HBISHOP_NUM + MAX_HROOK_NUM;
constexpr u32 MAX_FEATURES2_HAND_NUM
    = (int)color::NUM_COLORS * MAX_PIECES_IN_HAND_SUM;
...
public:
    ...
    {
        return (int)pos.isDraw(checkMaxPly);
    }

    int
    move(const int from_square, const int to_square, const bool promotion) const
    {
        if (promotion)
    ...
    {
        pos.setTurn((color::ColorEnum)turn);
    }
    int ply() const
    {
        return pos.gamePly();
    ...
    {
        pos.toHuffmanCodedPos((u8*)data);
    }
    void toPackedSfen(char* data) const
    {
        pos.toPackedSfen((u8*)data);
    }
    int piece(const int sq) const
    {
        return (int)pos.piece((Square)sq);
    }
    bool inCheck() const
    {
        return pos.inCheck();
    ...
    int mateMoveIn1Ply()
    {
        return pos.mateMoveIn1Ply().value();
    }
    ...
    {
        return pos.moveIsPseudoLegal(Move(move));
    }
    bool moveIsLegal(const int move) const
    {
        return pos.moveIsLegal(Move(move));
    }
    ...
    {
        typedef float features1_t[color::NUM_COLORS][MAX_FEATURES1_NUM]
                                 [SquareNum];
        typedef float features2_t[MAX_FEATURES2_NUM][SquareNum];
        const auto features1 = reinterpret_cast<features1_t* const>(mem1);
        const auto features2 = reinterpret_cast<features2_t* const>(mem2);
        float(*const features2_hand)[color::NUM_COLORS][MAX_PIECES_IN_HAND_SUM]
                                    [SquareNum]
            = reinterpret_cast<
                float(*const)[color::NUM_COLORS][MAX_PIECES_IN_HAND_SUM]
                             [SquareNum]>(mem2);
        ...
        for (color::ColorEnum c = color::BLACK; c < color::NUM_COLORS; ++c) {
            ...
            for (Square sq = SQ11; sq < SquareNum; ++sq) {
                ...
                // 利き数
                const int num = std::min(
                    MAX_ATTACK_NUM,
                    pos.attackersTo(c, sq, occupied_bb).popCount());
                for (int k = 0; k < num; k++) {
                    (*features1)[c2][PIECETYPE_NUM + PIECETYPE_NUM + k][sq2]
                        = 1;
                }
            }
            // hand
...
{
    return move & Move::PromoteFlag;
}
// 駒打ちか
bool __move_is_drop(const int move)
{
    return __move_from(move) >= 81;
}
// 移動する駒の種類
...
unsigned short __move16_from_psv(const unsigned short move16)
{
    ...
    return to | (from << 7)
           | ((move16 & MOVE_PROMOTE) != 0 ? Move::PromoteFlag : 0);
}
...
unsigned short __move16_to_psv(const unsigned short move16)
{
    ...
    return to | (from << 7) | drop
           | ((move16 & Move::PromoteFlag) != 0 ? MOVE_PROMOTE : 0);
}
...
int __dlshogi_make_move_label(const int move, const int color)
{
    ...
    if (from_sq < SquareNum) {
        // 白の場合、盤面を180度回転
        if ((color::ColorEnum)color == color::WHITE) {
...
public:
    ...
    __DfPn(
        const int max_depth, const uint32_t max_search_node, const int draw_ply)
