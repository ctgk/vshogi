...
inline int abs(const File f)
{
    return std::abs(static_cast<int>(f));
}
...
inline int abs(const Rank r)
{
    return std::abs(static_cast<int>(r));
}
...
inline Square operator+(const Square lhs, const SquareDelta rhs)
{
    return lhs + static_cast<Square>(rhs);
}
inline void operator+=(Square& lhs, const SquareDelta rhs)
{
    lhs = lhs + static_cast<Square>(rhs);
}
inline Square operator-(const Square lhs, const SquareDelta rhs)
{
    return lhs - static_cast<Square>(rhs);
...
inline bool isInFile(const File f)
{
    return (0 <= f) && (f < FileNum);
}
inline bool isInRank(const Rank r)
{
    return (0 <= r) && (r < RankNum);
}
// s が Square の中に含まれているか判定
inline bool isInSquare(const Square s)
{
...
inline bool isInSquare(const File f, const Rank r)
{
    return isInFile(f) && isInRank(r);
}
...
inline Direction squareRelation(const Square sq1, const Square sq2)
{
    return SquareRelation[sq1][sq2];
}

// 何かの駒で一手で行ける位置関係についての距離のテーブル。桂馬の位置は距離1とする。
extern int SquareDistance[SquareNum][SquareNum];
inline int squareDistance(const Square sq1, const Square sq2)
{
...
inline char fileToCharUSI(const File f)
{
    return '1' + f;
}
...
inline char fileToCharCSA(const File f)
{
    return '1' + f;
}
inline char rankToCharCSA(const Rank r)
{
    return '1' + r;
}
...
inline File charCSAToFile(const char c)
{
    return static_cast<File>(c - '1');
}
inline Rank charCSAToRank(const char c)
{
    return static_cast<Rank>(c - '1');
}
inline File charUSIToFile(const char c)
{
    return static_cast<File>(c - '1');
}
inline Rank charUSIToRank(const char c)
{
    return static_cast<Rank>(c - 'a');
}

// 後手の位置を先手の位置へ変換
inline constexpr Square inverse(const Square sq)
{
    return SquareNum - 1 - sq;
}
// 左右変換
inline constexpr File inverse(const File f)
{
    return FileNum - 1 - f;
}
// 上下変換
inline constexpr Rank inverse(const Rank r)
{
    return RankNum - 1 - r;
}
// Square の左右だけ変換
...
enum SquareWithWall : int32_t
{
    ...
    SQWW_U = DeltaN - (1 << 14) + (1 << 19),
    SQWW_D = -int(SQWW_U),
    SQWW_L = -int(SQWW_R),
    SQWW_RU = int(SQWW_R) + int(SQWW_U),
    SQWW_RD = int(SQWW_R) + int(SQWW_D),
    SQWW_LU = int(SQWW_L) + int(SQWW_U),
    SQWW_LD = int(SQWW_L) + int(SQWW_D),

    // SQ_11の地点に対応する値(他の升はこれ相対で事前に求めテーブルに格納)
    SQWW_11
    = SQ11 | (1 << 8) /* bit8 is 1 */ | (0 << 9) /*右に0升*/
...
// 型変換。下位8bit == Square
constexpr Square sqww_to_sq(SquareWithWall sqww)
{
    return Square(sqww & 0xff);
}

extern SquareWithWall sqww_table[SquareNum + 1];

// 型変換。Square型から。
static SquareWithWall to_sqww(Square sq)
{
    return sqww_table[sq];
}
...
enum Directions : uint8_t
{
    ...
    DIRECTIONS_LD = 128,
    DIRECTIONS_CROSS
    = DIRECTIONS_U | DIRECTIONS_D | DIRECTIONS_R | DIRECTIONS_L,
    DIRECTIONS_DIAG
    = DIRECTIONS_RU | DIRECTIONS_RD | DIRECTIONS_LU | DIRECTIONS_LD,
};
...
static Directions directions_of(Square sq1, Square sq2)
{
...
constexpr Directions to_directions(Direct d)
{
    return Directions(1 << d);
}

constexpr bool is_ok(Direct d)
{
    return DIRECT_ZERO <= d && d < DIRECT_NB_PLUS4;
}

// DirectをSquareWithWall型の差分値で表現したもの。
constexpr SquareWithWall DirectToDeltaWW_[DIRECT_NB] = {
    SQWW_RU,
    SQWW_R,
    SQWW_RD,
    SQWW_U,
...
constexpr SquareWithWall DirectToDeltaWW(Direct d)
{
