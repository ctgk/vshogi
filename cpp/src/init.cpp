...
// result  occupied
Bitboard
indexToOccupied(const int index, const int bits, const Bitboard& blockMask)
{
    ...
    for (File file = File1; file < FileNum; ++file) {
        for (Rank rank = Rank1; rank < RankNum; ++rank) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                Square sq2 = sq;
                while (true) {
                    if ((delta == DeltaNW || delta == DeltaNE)
                        && makeRank(sq2) == Rank1)
                        break;
                    if ((delta == DeltaSW || delta == DeltaSE)
                        && makeRank(sq2) == Rank9)
                        break;
                    if ((delta == DeltaNW || delta == DeltaSW)
                        && makeFile(sq2) == File9)
                        break;
                    if ((delta == DeltaNE || delta == DeltaSE)
                        && makeFile(sq2) == File1)
                        break;
                    sq2 += delta;
                    bb |= setMaskBB(sq2);
...
void initKingAttacks()
{
    for (Square sq = SQ11; sq < SquareNum; ++sq)
        KingAttack[sq]
            = rookAttack(sq, allOneBB()) | bishopAttack(sq, allOneBB());
}
...
void initKnightAttacks()
{
    for (color::ColorEnum c = color::BLACK; c < color::NUM_COLORS; ++c) {
        for (Square sq = SQ11; sq < SquareNum; ++sq) {
            ...
            if (bb)
                KnightAttack[c][sq]
                    = bishopStepAttacks(bb.constFirstOneFromSQ11())
                      & inFrontMask(c, makeRank(sq));
        }
    }
}
    ...
    for (Square sq1 = SQ11; sq1 < SquareNum; ++sq1) {
        ...
        for (Square sq2 = SQ11; sq2 < SquareNum; ++sq2) {
            ...
            if (file1 == file2)
                ...
                static_cast<int>(rank1 - rank2)
                == static_cast<int>(file1 - file2))
                SquareRelation[sq1][sq2] = DirecDiagNESW;
            else if (
                static_cast<int>(rank1 - rank2)
                == static_cast<int>(file2 - file1))
...
void initBetweenBB()
{
    for (Square sq1 = SQ11; sq1 < SquareNum; ++sq1) {
        for (Square sq2 = SQ11; sq2 < SquareNum; ++sq2) {
            ...
            if (direc & DirecCross)
                BetweenBB[sq1][sq2] = rookAttack(sq1, setMaskBB(sq2))
                                      & rookAttack(sq2, setMaskBB(sq1));
            else if (direc & DirecDiag)
                BetweenBB[sq1][sq2] = bishopAttack(sq1, setMaskBB(sq2))
                                      & bishopAttack(sq2, setMaskBB(sq1));
        }
    }
}
...
void initCheckTable()
{
    for (color::ColorEnum c = color::BLACK; c < color::NUM_COLORS; ++c) {
        ...
        for (Square sq = SQ11; sq < SquareNum; ++sq) {
            ...
            GoldCheckTable[c][sq].andEqualNot(
                setMaskBB(sq) | goldAttack(opp, sq));
        }
    }
    ...
    for (color::ColorEnum c = color::BLACK; c < color::NUM_COLORS; ++c) {
        ...
        for (Square sq = SQ11; sq < SquareNum; ++sq) {
            ...
            while (checkBB) {
                ...
                SilverCheckTable[c][sq]
                    |= (silverAttack(opp, checkSq) & TRank123BB);
            }
            ...
            while (checkBB) {
                const Square checkSq = checkBB.firstOneFromSQ11();
                SilverCheckTable[c][sq]
                    |= (silverAttack(opp, checkSq) & TRank4BB);
            }
            SilverCheckTable[c][sq].andEqualNot(
                setMaskBB(sq) | silverAttack(opp, sq));
        }
    }
    ...
    for (color::ColorEnum c = color::BLACK; c < color::NUM_COLORS; ++c) {
        ...
        for (Square sq = SQ11; sq < SquareNum; ++sq) {
            ...
            LanceCheckTable[c][sq].andEqualNot(
                setMaskBB(sq) | pawnAttack(opp, sq));
        }
    }
    ...
    for (color::ColorEnum c = color::BLACK; c < color::NUM_COLORS; ++c) {
        ...
        for (Square sq = SQ11; sq < SquareNum; ++sq) {
            ...
            while (checkBB) {
                const Square checkSq = checkBB.firstOneFromSQ11();
                // 移動元が敵陣 == 成れる == 王の動き
                BishopCheckTable[c][sq]
                    |= bishopAttack(checkSq, allZeroBB()) & TRank123BB;
            }
    ...
    for (Square sq0 = SQ11; sq0 < SquareNum; ++sq0) {
        for (Square sq1 = SQ11; sq1 < SquareNum; ++sq1) {
            ...
            case DirecMisc:
                ...
                break;
            case DirecFile:
                SquareDistance[sq0][sq1] = abs(
                    static_cast<int>(sq0 - sq1) / static_cast<int>(DeltaN));
                break;
            case DirecRank:
                SquareDistance[sq0][sq1] = abs(
                    static_cast<int>(sq0 - sq1) / static_cast<int>(DeltaE));
                break;
            case DirecDiagNESW:
                SquareDistance[sq0][sq1] = abs(
                    static_cast<int>(sq0 - sq1) / static_cast<int>(DeltaNE));
                break;
            case DirecDiagNWSE:
                SquareDistance[sq0][sq1] = abs(
                    static_cast<int>(sq0 - sq1) / static_cast<int>(DeltaNW));
                break;
            default:
                UNREACHABLE;
            }
