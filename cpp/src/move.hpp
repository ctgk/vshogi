...
public:
    ...
    const Move& operator=(const Move& m) volatile
    {
        value_ = m.value_;
        return m;
    }
...
inline void insertionSort(T first, T last)
{
    if (UseSentinel)
        assert(
            std::max_element(first - 1, last)
            == first - 1); // 番兵が最大値となることを確認
    if (first != last) {
        for (T curr = first + 1; curr != last; ++curr) {
            if (*(curr - 1) < *curr) {
                ...
                while ((UseSentinel || curr != first) && *(curr - 1) < tmp);
                *curr = std::move(tmp);
            }
        }
    }
