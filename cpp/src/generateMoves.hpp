...
public:
    explicit MoveList(const Position& pos)
        : curr_(moveList_), last_(generateMoves<MT>(moveList_, pos))
    {
    }
    void operator++()
    {
    ...
    {
        return (curr_ == last_);
    } // 通常のコンテナの begin() と end() の関係では無いので注意。
    Move move() const
    {
        return curr_->move;
    }
    ...
    const Position& pos)
{
    ...
    assert(
        !((pt == piece::KI || pt == piece::OU || MT == Drop) && PM == Promote));
    Move move
        = ((MT == NonCapture || MT == NonCaptureMinusPro)
               ? makeMove(pt, from, to)
               : makeCaptureMove(pt, from, to, pos));
    if (PM == Promote)
        move |= promoteFlag();
    return move;
}
