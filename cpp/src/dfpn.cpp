...
public:
    explicit MovePicker(const Position& pos)
    {
        if (or_node) {
            ...
            while (curr != last_) {
                if (!pos.pseudoLegalMoveIsLegal<false, false>(
                        curr->move, pinned))
                    curr->move = (--last_)->move;
                else
                    ++curr;
            ...
            for (i++; i < entries.size(); i++) {
                ...
                if (entry_rest.pn == 0) {
                    if (hand.isEqualOrSuperior(entry_rest.hand)
                        && entry_rest.num_searched != REPEAT) {
                        return entry_rest;
                    }
...
template <bool or_node>
void DfPn::dfpn_inner(
    Position& n,
    const int thpn,
    const int thdn /*, bool inc_flag*/,
    const uint16_t maxDepth,
    uint32_t& searchedNode)
{
    ...
    for (const auto& move : move_picker) {
        ...
        transposition_table.GetChildFirstEntry<or_node>(
            n, move, ttkey.entries, ttkey.hand);
    }
    ...
    while (searchedNode < maxSearchNode && !stop) {
        ...
        if (or_node) {
            ...
            for (const auto& move : move_picker) {
                auto& ttkey = ttkeys[&move - move_picker.begin()];
                const auto& child_entry = transposition_table.LookUpDirect(
        ...
        ++searchedNode;
        dfpn_inner<!or_node>(
            n, thpn_child, thdn_child /*, inc_flag*/, maxDepth, searchedNode);
...
Move DfPn::dfpn_move(Position& pos)
{
    ...
    for (const auto& move : move_picker) {
        const auto& child_entry
            = transposition_table.LookUpChildEntry<true>(pos, move);
        if (child_entry.pn == 0) {
            return move;
        }
    }
...
int DfPn::get_pv_inner(Position& pos, std::vector<u32>& pv)
{
    if (or_node) {
        ...
        for (const auto& move : move_picker) {
            const auto& child_entry
                = transposition_table.LookUpChildEntry<false>(pos, move);
            if (child_entry.pn == 0) {
...
bool DfPn::dfpn(Position& r)
{
    ...
    if (!r.inCheck()) {
        ...
        if (mate1ply) {
            auto& child_entry
                = transposition_table.LookUpChildEntry<true>(r, mate1ply);
            child_entry.pn = 0;
    ...
    dfpn_inner<true>(
        ...
        kInfinitePnDn,
        kInfinitePnDn /*, false*/,
        std::min(r.gamePly() + kMaxDepth, draw_ply),
        searchedNode);
