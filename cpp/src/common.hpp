...
#else
inline int
count1s(u64 x) //任意の値の1のビットの数を数える。( x is not a const value.)
{
...
template <typename T>
inline std::string
putb(const T value, const int msb = sizeof(T) * 8 - 1, const int lsb = 0)
{
    std::string str;
...
template <typename T>
Eraser& operator<<(Eraser& temp, const T&)
{
    return temp;
}
#endif
...
struct HashTable
{
    ...
    {
        return entries_ + (static_cast<size_t>(k) & (Size - 1));
    }
    void clear()
    {
...
public:
    ...
    int elapsed() const
    {
        ...
        return static_cast<int>(
            duration_cast<milliseconds>(std::chrono::system_clock::now() - t_)
                .count());
    }
...
{
    static constexpr KeyType index(const KeyType i, const KeyType j)
    {
        return i * (i + 1) / 2 + j;
    }
    static constexpr size_t Size
        = index((KeyType)(Size_i - 1), (KeyType)(Size_j - 1)) + 1;
    const ElementType& at(const KeyType i, const KeyType j) const
    {
        return (i < j ? data_[index(i, j)] : data_[index(j, i)]);
    }
    ElementType& at(const KeyType i, const KeyType j)
    {
        return (i < j ? data_[index(i, j)] : data_[index(j, i)]);
    }
