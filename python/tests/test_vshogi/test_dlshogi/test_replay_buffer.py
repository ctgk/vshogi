...
def test_averagize():
    ...
    assert np.isclose(
        buffer.get_average_of("rbsgk/4p/5/P4/KGSBR b -").value01, 0.25
    )
    ...
    assert np.isclose(
        buffer.get_average_of("rbsgk/4p/5/P4/KGSBR b -").value01, 0.5
    )
    assert np.isclose(buffer.get_average_of("4k/4p/5/P4/K4 b -").value01, 1.0)
