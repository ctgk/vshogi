...
void test_parser()
{
    ...
    parser.parse_csa_file(
        R"(R:\test\wdoor+floodgate-600-10+catshogi+tanuki-_5500U+20151011200002.csa)");
...
{
    parser::__Parser parser;
    parser.parse_csa_file("R:\\test\\wdoor+floodgate-300-10F+15gou_i3_2c+"
                          "ArgoCorse1.10b+20180204233002.csa");
...
void test_psv()
{
    ...
    std::ifstream ifs(
        R"(F:\yane\shuffled_sfen1_100.bin)",
    ...
    ifs.read(
        reinterpret_cast<char*>(psvvec), sizeof(PackedSfenValue) * entryNum);
    ifs.close();
